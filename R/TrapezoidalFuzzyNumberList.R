#' @title 'TrapezoidalFuzzyNumberList' is a child class of 'StatList'. It contains
...
TrapezoidalFuzzyNumberList <- R6::R6Class(
  ...
  private = list (
    ...
    inf = function(x = NA,
                   i = NA,
                   list = NA) {
      return (list$numbers[[i]]$getInf0() + x * (list$numbers[[i]]$getInf1() - list$numbers[[i]]$getInf0()))
    },
    ...
    sup = function(x = NA,
                   i = NA,
                   list = NA) {
      return (list$numbers[[i]]$getSup0() + x * (list$numbers[[i]]$getSup1() - list$numbers[[i]]$getSup0()))
    },
    ...
    extractMatrix = function() {
      F <- matrix(ncol = 4)
      for (i in 1:length(self$numbers)) {
        F <-
          rbind(
            F,
            c(
              self$numbers[[i]]$getInf0(),
              self$numbers[[i]]$getInf1(),
              self$numbers[[i]]$getSup1(),
              self$numbers[[i]]$getSup0()
            )
          )
      }
      return (F)
  ...
  public = list(
    #' @field numbers is a collection of trapezoidal fuzzy numbers.
    numbers = NULL,
    ...
    #'
    #' @return The TrapezoidalFuzzyNumberList object created with all attributes set.
    #'
    #' @examples
    ...
    add = function(s = NA,
      ...
      if (self$checking() &&
          s$checking() && (length(s$numbers) == 1)) {
        u <- NULL
        if (class(s)[1] == "FuzzyNumberList") {
          u <-
            self$transfTra(as.integer(nrow(s$numbers[[1]]$getFNAlphaLevels())))
        } else{
    ...
    #' @details See examples.
    #'
    #' @return TRUE if all TrapezoidalFuzzyNumbers are valid. Otherwise, FALSE.
    #'
    #' @examples
    #' # Example 1:
    ...
    checking = function(verbose = TRUE) {
      for (val in 1:length(self$numbers)) {
        if (!self$numbers[[val]]$is_valid()) {
          if (verbose) {
            print("Not all trapezoidal fuzzy numbers are valid")
          }
          return (FALSE)
        }
      }
    ...
    #'
    #' @param s TrapezoidalFuzzyNumberList containing TrapezoidalFuzzyNumbers characterized
    ...
    dthetaphi = function(s = NA,
      ...
      if (self$checking() && s$checking()) {
        ...
        for (i in 1:r) {
          for (j in 1:p) {
            c[i, j] <-
              ((self$numbers[[i]]$getInf0() + self$numbers[[i]]$getSup0()) -
                 (s$numbers[[j]]$getInf0() + s$numbers[[j]]$getSup0())
              ) / 2

            d[i, j] <-
              ((self$numbers[[i]]$getSup0() - self$numbers[[i]]$getInf0()) -
                 (s$numbers[[j]]$getSup0() - s$numbers[[j]]$getInf0())
              ) / 2

            e[i, j] <-
              ((self$numbers[[i]]$getInf1() + self$numbers[[i]]$getSup1()) -
                 (s$numbers[[j]]$getInf1() + s$numbers[[j]]$getSup1())
              ) / 2 - c[i, j]

            f[i, j] <-
              ((self$numbers[[i]]$getSup1() - self$numbers[[i]]$getInf1()) -
                 (s$numbers[[j]]$getSup1() - s$numbers[[j]]$getInf1())
              ) / 2 - d[i, j]
    ...
    #' @details See examples.
    #'
    #' @return the Gini-Simpson diversity index, which is a real number. If the body's
    #' method inner conditions are not met, NA will be returned.
    #'
    #' @examples
    #' # Example 1:
    ...
    #' @details See examples.
    #'
    #' @return the value of the M-estimator of scale, which is a real number. If the
    ...
    #'
    #' @param nl positive integer, by default nl=101. It indicates the number of desired
    ...
    median1Norm = function(nl = 101L) {
      ...
      if (self$checking()) {
        ...
        l <- length(fuzzyArray$numbers)
        F <- array(dim = c(nl, 3, l))
        for (i in 1:l) {
          F[, , i] <- fuzzyArray$numbers[[i]]$getFNAlphaLevels()
        }
        ...
        return(FuzzyNumberList$new(c(FuzzyNumber$new(
          array(c(alpha, result[, 2, 1], result[, 3, 1]), dim = c(length(result[, 1, 1]), 3))
        ))))
      }
    ...
    medianWabl = function(nl = 101L,
      ...
      if (self$checking()) {
        ...
        return(FuzzyNumberList$new(c(FuzzyNumber$new(
          array(c(median[, 1, 1], median[, 2, 1], median[, 3, 1]), dim = c(length(median[, 1, 1]), 3))
        ))))
      }
    ...
    #' @details See examples.
    #'
    #' @return the scale measure Qn, which is a real number. If the body's
    #' method inner conditions are not met, NA will be returned.
    #'
    #' @examples
    #' # Example 1:
    ...
    #'
    #' @param s TrapezoidalFuzzyNumberList containing TrapezoidalFuzzyNumbers
    ...
    rho1 = function(s = NA) {
      ...
      if (self$checking() && s$checking()) {
        ...
        for (i in 1:r) {
          for (j in 1:p) {
            c[i, j] <-
              self$numbers[[i]]$getInf0() - s$numbers[[j]]$getInf0()


            d[i, j] <-
              self$numbers[[i]]$getInf1() - s$numbers[[j]]$getInf1()


            e[i, j] <-
              self$numbers[[i]]$getSup1() - s$numbers[[j]]$getSup1()


            f[i, j] <-
              self$numbers[[i]]$getSup0() - s$numbers[[j]]$getSup0()
    ...
    #' @details See examples.
    #'
    #' @return the scale measure Sn, which is a real number. If the body's
    #' method inner conditions are not met, NA will be returned.
    #'
    #' @examples
    #' # Example 1:
    ...
    #' @details See examples.
    #'
    #' @return the scale measure Tn, which is a real number. If the body's
    #' method inner conditions are not met, NA will be returned.
    #'
    #' @examples
    #' # Example 1:
    ...
    transfTra = function(nl = 101L) {
      stopifnot(typeof(nl) == "integer" && nl > 0)
